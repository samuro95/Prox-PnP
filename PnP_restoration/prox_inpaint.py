...
def inpaint():
    ...
    for i in range(min(len(input_paths), hparams.n_images)): # For each image
        ...
        if hparams.extract_images or hparams.extract_curves or hparams.print_each_step:
            inpainted_im, output_psnr, output_psnrY, x_list, z_list, y_list, Dg_list, psnr_tab, Dx_list, g_list, F_list, Psi_list = PnP_module.restore(mask_im, init_im, input_im, mask, extract_results=True)
        else:
