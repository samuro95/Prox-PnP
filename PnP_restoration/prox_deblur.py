...
def deblur():
    ...
    # Hyper-parameters
    if hparams.PnP_algo == 'pGSPnP-DRS':
        hparams.alpha = 0.5
    ...
    print(
        '\n ' + PnP_module.hparams.PnP_algo + ' deblurring with image sigma:{:.3f}, model sigma:{:.3f}, lamb:{:.3f} \n'.format(
    ...
    for k_index in k_list: # For each kernel
        ...
        for i in range(min(len(input_paths),hparams.n_images)): # For each image
            ...
            if hparams.extract_images:
                ...
                save_im_path = os.path.join(save_im_path, PnP_algo)
                if not os.path.exists(save_im_path):
                    os.mkdir(save_im_path)
